...
public function store(CitizenDetailsRequest $request)
{
    ...
    $validated = $request->validate([
        ...
        'services_availed' => 'nullable|array', // Array of service IDs
        'services_availed.*' => 'nullable|exists:services,id', // Ensure each ID exists in the services table
    ]);

    // Get the last citizen_id that starts with '200-'
    $lastCitizen = CitizenDetails::where('citizen_id', 'like', '200-%')
        ->orderBy('citizen_id', 'desc')
        ->first();

    // Extract the incrementing number, remove the '200-' prefix
    ...
    // Add citizen_id to the validated data
    $validated['citizen_id'] = $newCitizenId;

    // Create the citizen record with the validated data and generated citizen_id
    ...
    // Attach the availed services to the citizen history
